...
/**
 * Pure timeline state/controller for animation playback.
 * Keeps loop behavior separate from the Compose UI so it can be tested safely.
 */
...
fun TimelineState.nextFrame(frameCount: Int): TimelineState {
    ...
    when {
        ...
        loopMode == TimelineLoopMode.PING_PONG -> {
            nextDirection = -direction
            next = frame + nextDirection
        }
        else -> {
            next = if (next < 0) 0 else frameCount - 1
            stillPlaying = false
        }
    }
...
fun TimelineState.frameDelayMillis(): Long =
    (1000f / (fps.coerceIn(1, 60) * speed.coerceIn(0.1f, 4f))).toLong().coerceAtLeast(1L)
