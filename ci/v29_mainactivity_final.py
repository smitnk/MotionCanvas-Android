...
\n# Add the MotionCanvas OpenToonz-inspired thick-point stroke engine as a standalone source file.\nengine_path = Path("build-source/app/src/main/java/com/smitnk/motioncanvas/drawing/OpenToonzStrokeEngine.kt")\nengine_path.parent.mkdir(parents=True, exist_ok=True)\nengine_path.write_text(r'''package com.smitnk.motioncanvas.drawing\n\nimport kotlin.math.max\n\ndata class OTThickPoint(val x: Float, val y: Float, val thick: Float)\ndata class OTQuadraticChunk(val p0: OTThickPoint, val p1: OTThickPoint, val p2: OTThickPoint)\n\n/** OpenToonz-inspired stroke representation; implementation is original MotionCanvas code. */\nclass OpenToonzStrokeEngine {\n    private val controlPoints = mutableListOf<OTThickPoint>()\n    private val chunks = mutableListOf<OTQuadraticChunk>()\n\n    fun clear() { controlPoints.clear(); chunks.clear() }\n\n    fun addRawPoint(x: Float, y: Float, pressure: Float) {\n        val thickness = if (pressure > 0f) max(1f, pressure * 28f) else 12f\n        controlPoints += OTThickPoint(x, y, thickness)\n        rebuildChunks()\n    }\n\n    fun finalizeStroke() {\n        if (controlPoints.size < 2) return\n        val simplified = mutableListOf<OTThickPoint>()\n        simplified += controlPoints.first()\n        val step = max(2, controlPoints.size / 8)\n        var i = step\n        while (i < controlPoints.lastIndex) { simplified += controlPoints[i]; i += step }\n        simplified += controlPoints.last()\n        controlPoints.clear(); controlPoints.addAll(simplified); rebuildChunks()\n    }\n\n    fun rebuildChunks() {\n        chunks.clear()\n        if (controlPoints.size < 2) return\n        if (controlPoints.size == 2) {\n            val a = controlPoints[0]; val b = controlPoints[1]\n            chunks += OTQuadraticChunk(a, OTThickPoint((a.x+b.x)/2f,(a.y+b.y)/2f,(a.thick+b.thick)/2f), b)\n            return\n        }\n        for (i in 0 until controlPoints.lastIndex) {\n            val a = controlPoints[i]; val b = controlPoints[i+1]\n            chunks += OTQuadraticChunk(a, OTThickPoint((a.x+b.x)/2f,(a.y+b.y)/2f,(a.thick+b.thick)/2f), b)\n        }\n    }\n\n    fun updatePoint(index: Int, x: Float, y: Float) {\n        if (index !in controlPoints.indices) return\n        val p = controlPoints[index]\n        controlPoints[index] = p.copy(x=x, y=y)\n        rebuildChunks()\n    }\n\n    fun points(): List<OTThickPoint> = controlPoints.toList()\n    fun chunks(): List<OTQuadraticChunk> = chunks.toList()\n}\n\np.write_text(s)
