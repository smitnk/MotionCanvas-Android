...
# Final parser-simulation repairs.
p = root / "MainActivity.kt"
s = p.read_text()
s = s.replace('''        topBar = {
            if (workspaceVisibility.topBar) {
            TopAppBar(''','''        topBar = {
            TopAppBar(''',1)
s = s.replace('''                    FilterChip(
                        selected = referenceEditMode,
                        onClick = { if (referenceBitmap != null) referenceEditMode = !referenceEditMode },
                        label = { Text("Ref Edit") }
                    )
                    if (workspaceVisibility.frameToolsWidget) {''','''                    FilterChip(
                        selected = referenceEditMode,
                        onClick = { if (referenceBitmap != null) referenceEditMode = !referenceEditMode },
                        label = { Text("Ref Edit") }
                    )
                    }
                    if (workspaceVisibility.frameToolsWidget) {''',1)
...
s = s.replace('''                }
    ...
    if (showAdvancedPanel) {''',1)
marker = '''    val recordPermissionLauncher = rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
        if (granted && !isRecording) { recorder.start(); isRecording = true }
    }
'''
decls = '''    var waveform by remember { mutableStateOf<List<Float>>(emptyList()) }
...
'''
if "var waveform by remember { mutableStateOf<List<Float>>(emptyList()) }\n    var rotoscopeFrames" not in s:
    s = s.replace(marker, marker + decls, 1)
s = s.replace('''    var waveform by remember { mutableStateOf<List<Float>>(emptyList()) }
    var showProTools by remember { mutableStateOf(false) }
    var rotoscopeFrames by remember { mutableStateOf<List<ImageBitmap>>(emptyList()) }
    var rotoscopeMaxFrames by remember { mutableIntStateOf(60) }
''','''    var showProTools by remember { mutableStateOf(false) }
''',1)
s = s.replace('''            Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){ Button(onClick=if(isRecording) onStopRecording else onStartRecording){Text(if(isRecording) "Stop" else "Record")} OutlinedButton(onClick=onPreview,enabled=project.audioPath!=null){Text("Preview")} OutlinedButton(onClick=onStopPreview){Text("Stop")}}''','''            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Button(onClick = { if (isRecording) onStopRecording() else onStartRecording() }) {
                    Text(if (isRecording) "Stop" else "Record")
                }
                OutlinedButton(onClick = onPreview, enabled = project.audioPath != null) {
                    Text("Preview")
                }
...
s = s.replace("TextPrimary","White")
p.write_text(s)
kp = root / "animation/KeyframeInterpolator.kt"
kp.write_text(r'''package com.smitnk.motioncanvas.animation
import androidx.compose.ui.geometry.Offset
...
''')
print("V29 final parser repairs completed")
